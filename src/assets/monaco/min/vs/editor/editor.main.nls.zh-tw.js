...
define("vs/editor/editor.main.nls.zh-tw", {
  ...
  "vs/editor/common/config/commonEditorConfig": [
    ...
    "行號以目前游標的相對值顯示。",
    "每 10 行顯示行號。",
    "控制行號的顯示方式。允許的設定值為 'on'、 'off'、'relative' 及 'interval'",
    ...
    "控制編輯器是否會使用動畫捲動",
    "控制是否會顯示迷你地圖",
    "控制要轉譯迷你地圖的方向。可能的值為 'right' 與 'left'",
    ...
    "對應 Windows 和 Linux 的 `Control` 與對應 macOS 的 `Command`",
    "對應 Windows 和 Linux 的 `Alt` 與對應 macOS 的 `Option`",
    "用於新增多個滑鼠游標的修改。 `ctrlCmd` 會對應到 Windows 和 Linux 上的 `Control` 以及 macOS 上的 `Command`。 [移至定義] 及 [開啟連結] 滑鼠手勢將會適應以避免和 multicursor 修改衝突。",
    "允許在字串內顯示即時建議。",
    "允許在註解中顯示即時建議。",
    "允許在字串與註解以外之處顯示即時建議。",
    "控制是否應在輸入時自動顯示建議",
    ...
    "控制當使用者輸入, 貼上或移行時，編輯器應自動調整縮排。語言的縮排規則必須可用",
    "控制輸入觸發字元時，是否應自動顯示建議",
    ...
    "控制是否應根據文件中的單字計算自動完成。",
    "一律選取第一個建議。",
    "除非進一步的鍵入選取一個建議，否則選取最近的建議。例如，因為 `log` 最近完成，所以 `console.| -> console.log`。",
    "根據先前已完成這些建議的首碼選取建議。例如，`co -> console` 與 `con -> const`。",
    "控制在顯示建議清單時如何預先選取建議。",
    "建議小工具的字型大小",
    "建議小工具的行高",
  ...
  "vs/editor/common/view/editorColorRegistry": [
    "目前游標位置行的反白顯示背景色彩。",
    "目前游標位置行之周圍框線的背景色彩。",
    "突顯顯示範圍的背景顏色，例如快速開啟和尋找功能。不能使用非透明的顏色來隱藏底層的樣式。",
    "反白顯示範圍周圍邊框的背景顏色。",
    "編輯器游標的色彩。",
    ...
    "編輯器內提示訊息的提示線前景色彩",
    "編輯器內提示訊息的提示線邊框色彩",
    "概述反白範圍的尺規標記顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "錯誤的概觀尺規標記色彩。",
    "警示的概觀尺規標記色彩。",
    "資訊的概觀尺規標記色彩。"
  ],
  ...
  "vs/editor/contrib/hover/hover": ["動態顯示"],
  "vs/editor/contrib/hover/modesContentHover": ["正在載入..."],
  ...
  "vs/editor/contrib/referenceSearch/peekViewWidget": ["關閉"],
  "vs/editor/contrib/referenceSearch/referenceSearch": [
    " - {0} 個參考",
    "尋找所有參考"
  ],
  "vs/editor/contrib/referenceSearch/referencesController": ["正在載入..."],
  ...
  "vs/editor/contrib/smartSelect/smartSelect": ["展開選取", "縮小選取"],
  ...
  "vs/editor/contrib/wordHighlighter/wordHighlighter": [
    "讀取存取符號時的背景顏色，如讀取變數。不能使用非透明的顏色來隱藏底層的樣式。",
    "寫入存取符號時的背景顏色，如寫入變數。不能使用非透明的顏色來隱藏底層的樣式。",
    "讀取存取期間 (例如讀取變數時) 符號的邊框顏色。",
    "寫入存取期間 (例如寫入變數時) 符號的邊框顏色。 ",
    "概述反白符號的尺規標記顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "概述反白寫入權限符號的尺規標記顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "移至下一個反白符號",
    "移至上一個反白符號"
  ],
  ...
  "vs/platform/list/browser/listService": [
    ...
    "透過滑鼠多選，用於在樹狀目錄與清單中新增項目的輔助按鍵 (例如在總管中開啟 [編輯器] 及 [SCM] 檢視)。在 Windows 及 Linux 上，`ctrlCmd` 對應 `Control`，在 macOS 上則對應 `Command`。[在側邊開啟] 滑鼠手勢 (若支援) 將會適應以避免和多選輔助按鍵衝突。",
    "以滑鼠按一下開啟項目。",
    "以滑鼠按兩下開啟項目。",
    "控制如何使用滑鼠在樹狀目錄與清單中開啟項目 (若有支援)。設為 `singleClick` 可以滑鼠按一下開啟物件，設為 `doubleClick` 則只能透過按兩下滑鼠開啟物件。對於樹狀目錄中具子系的父系而言，此設定會控制應以滑鼠按一下或按兩下展開父系。注意，某些樹狀目錄或清單若不適用此設定則會予以忽略。",
    "控制是否支援工作台中的水平滾動。"
  ],
  ...
  "vs/platform/theme/common/colorRegistry": [
    ...
    "編輯器選取範圍的色彩。",
    "為選取的文字顏色高對比化",
    "在非使用中的編輯器的選取項目顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "與選取項目相同的區域顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "選取時，內容相同之區域的框線色彩。",
    "符合目前搜尋的色彩。",
    "符合搜尋條件的其他項目的顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "為限制搜索的範圍著色。不能使用非透明的顏色來隱藏底層的樣式。",
    "符合目前搜尋的框線色彩。",
    "符合其他搜尋的框線色彩。",
    "限制搜尋範圍的邊框顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "突顯懸停顯示的文字。不能使用非透明的顏色來隱藏底層的樣式。",
    "編輯器動態顯示的背景色彩。",
    "編輯器動態顯示的框線色彩。",
    "使用中之連結的色彩。",
    ...
    "插入的文字外框色彩。",
    "移除的文字外框色彩。",
    "目前內嵌合併衝突中的深色標題背景。不能使用非透明的顏色來隱藏底層的樣式。",
    "目前內嵌合併衝突中的內容背景。不能使用非透明的顏色來隱藏底層的樣式。",
    "傳入內嵌合併衝突中的深色標題背景。不能使用非透明的顏色來隱藏底層的樣式。",
    "傳入內嵌合併衝突中的內容背景。不能使用非透明的顏色來隱藏底層的樣式。",
    "內嵌合併衝突中的共同始祖標題背景。不能使用非透明的顏色來隱藏底層的樣式。",
    "內嵌合併衝突中的共同始祖內容背景。不能使用非透明的顏色來隱藏底層的樣式。",
    "內嵌合併衝突中標頭及分隔器的邊界色彩。",
    "目前內嵌合併衝突的概觀尺規前景。",
    "傳入內嵌合併衝突的概觀尺規前景。",
    "內嵌合併衝突中的共同上階概觀尺規前景。",
    "概述符合尋找條件的尺規標記顏色。不能使用非透明的顏色來隱藏底層的樣式。",
    "概述反白選擇的尺規標記顏色。不能使用非透明的顏色來隱藏底層的樣式。"
  ],
